...
local Hook
Hook = hookfunction(Network.FireServer, newcclosure(function(...)
   local args = {...}
...
for i,v in pairs(game.Players:GetDescendants()) do
   if string.find(tostring(v.Name):lower(), 'guihandler') then
       v:Destroy()
   end
end
...
wait()
pcall(function()
if game:GetService'Players'.LocalPlayer.Character:FindFirstChild("Shield") then
...
debug.getupvalue(getsenv(game:GetService("Players").LocalPlayer.PlayerGui.MainGui.MainLocal).Reload, 3).Debounce = value

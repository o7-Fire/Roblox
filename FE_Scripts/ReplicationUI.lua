...
_player = {
    ...
    b = function() return _player.p:FindFirstChild("Backpack") end,
    c = function() return _player.p.Character end
}
_functions = {
    respawn = function()
        local c = _player.c()
    ...
    tools = function(int)
        ...
        local savepos = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                            .CFrame
        local renderstepped = runservice.RenderStepped:Connect(
                ...
                for _, char in pairs(workspace:GetChildren()) do
                    if char.Name == LocalPlayer.Name and
                ...
                repeat runservice.RenderStepped:Wait() until LocalPlayer.Character ~=
                    nil
                lastchar = char
            end)
        repeat runservice.RenderStepped:Wait() until #LocalPlayer:GetChildren() -
            3 - currentamount >= toolamount
        renderstepped:Disconnect()
        repeat runservice.RenderStepped:Wait() until LocalPlayer.Character:FindFirstChild(
            "HumanoidRootPart") ~= nil
        lastchar:Destroy()

        utilityElements.Tools.Text = "Done respawning?"
        repeat runservice.RenderStepped:Wait() until utilityElements.Tools.Text ==
            "Load Tools"
    ...
    load = function()
        ...
        for i, Model in pairs(c:GetChildren()) do
            if Model:IsA("Model") and Model.Name == "gay" then
                for i, v in pairs(Model:GetDescendants()) do
                    ...
                    if not v:IsA("BasePart") and not v:IsA("JointInstance") and
                        not v:IsA("BodyMover") and not v:isA("Humanoid") and
                        not v:IsA("Model") then v:Remove() end
                end
                Model.ChildAdded:Connect(
                    function(c) c.Parent = nil end)
            end
        end
        ...
        wrap(function()
            ...
            connection1 = game:GetService("RunService").RenderStepped:Connect(
                              function(step)
                    for i = 1, amt do
                        local tool = tools[i]
                        if parts[i] then
                    ...
                    if _player.c() ~= c or
                        c:FindFirstChildWhichIsA("Humanoid").Health == 0 then
                        connection1:Disconnect()
                    end

                    utilityElements.Replicate.Text =
                        ("Replicate (%dfps)"):format(1 / step)
                end)
        end)

        local connection2
        connection2 = game:GetService("RunService").Heartbeat:Connect(
                          function()
                for i = 1, amt do
                    local tool = tools[i]
                    if parts[i] then
                        if parts[i].Transparency ~= 1 then
                            local p = _cframe.toWorldPoint(parts[i].CFrame) *
                                          offset
                            grip(tool, p)
                        else
                            local p = _cframe.toWorldPoint(
                                          CFrame.new(0, -250, 0))
                            grip(tool, p)
                        end
                    end
...
_cframe = {
    diff = function(a, b) return (a:Inverse() * b) end,
...
scriptList = {
    Character = {
        ...
        ['Winged Titan'] = {
            "rbxassetid://3593786550", "rbxassetid://3070358214"
        },
        ['Part Surround'] = {
            "rbxassetid://3546680077", "rbxassetid://3546673786"
        }
    },
    Mechanical = {
    ...
    Aesthetic = {
        ...
        ['Star Glitcher'] = {
            "rbxassetid://3074734519", "rbxassetid://3074751071"
        },
...
wrap = function(f) return coroutine.wrap(f)() end
    ...
    if v:IsA("TextButton") then
        ...
        pageLayout.Changed:Connect(function(p)
            if pageLayout.CurrentPage.Name == v.Name then
                guiLib.tween(v, "Sine", "Out", .5,
                             {BackgroundColor3 = Color3.fromRGB(255, 118, 26)})
            else
                guiLib.tween(v, "Sine", "Out", .5,
                             {BackgroundColor3 = Color3.fromRGB(236, 109, 17)})
            end
        end)
    end
end
...
utilityElements.Reset.MouseButton1Click:Connect(
    function()
        ...
        for i, v in pairs(_player.p:GetChildren()) do
            if v:IsA("Tool") then v.Parent = _player.b() end
        end
    end)

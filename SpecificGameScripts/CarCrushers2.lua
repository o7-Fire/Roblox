...
do
                ...
                if (not gpe) then
                    if key.KeyCode == Enum.KeyCode.RightControl then
                        ...
                        for i, data in next, library.queue do
                            local pos = (library.toggled and data.p or
                                            UDim2.new(-1, 0, -0.5, 0))
                            data.w:TweenPosition(pos,
                                                 (library.toggled and 'Out' or
                                                     'In'), 'Quad', 0.15, true)
                            wait();
                        end
                    end
    ...
    do
        ...
        function types.window(name, options)
            ...
            local newWindow = library:Create('Frame', {
                ...
                library:Create("Frame", {
                    ...
                    BackgroundColor3 = (options.underlinecolor ~= "rainbow" and
                        options.underlinecolor or Color3.new()),
                    BorderSizePixel = 0,
                    ZIndex = 3
                }),
                library:Create('Frame', {
            ...
            if options.underlinecolor == "rainbow" then
                table.insert(library.rainbowtable,
                             newWindow:FindFirstChild('Underline'))
            end
            ...
            table.insert(library.queue,
                         {w = window.object, p = window.object.Position})

            newWindow:FindFirstChild("window_toggle").MouseButton1Click:connect(
                function()
                    window.toggled = not window.toggled;
                    newWindow:FindFirstChild("window_toggle").Text =
                        (window.toggled and "+" or "-")
                    ...
                    local targetSize = window.toggled and
                                           UDim2.new(1, 0, 0, y + 5) or
                                           UDim2.new(1, 0, 0, 0);
                    local targetDirection = window.toggled and "In" or "Out"

                    window.container:TweenSize(targetSize, targetDirection,
                                               "Quad", 0.15, true)
                    wait(.15)
        ...
        function types:Label(text)
            local v = game:GetService 'TextService':GetTextSize(text, 18,
                                                                Enum.Font
                                                                    .SourceSans,
                                                                Vector2.new(
                                                                    math.huge,
        ...
        function types:Toggle(name, options, callback)
            ...
            local function click(t)
                ...
                check:FindFirstChild(name).Checkmark.Text =
                    location[flag] and utf8.char(10003) or "";
            end
        ...
        function types:Bind(name, options, callback)
            ...
            local nm =
                (default and (shortNames[default.Name] or default.Name) or
                    "None");
            local check = library:Create('Frame', {
                BackgroundTransparency = 1,
            ...
            button.MouseButton1Click:connect(
                function()
                    ...
                    if (a.UserInputType ~= Enum.UserInputType.Keyboard and
                        (allowed[a.UserInputType.Name]) and (not keyboardOnly)) or
                        (a.KeyCode and (not banned[a.KeyCode.Name])) then
                        local name = (a.UserInputType ~=
                                         Enum.UserInputType.Keyboard and
                                         a.UserInputType.Name or a.KeyCode.Name);
                        location[flag] = (a);
            ...
            if location[flag] then
                button.Text = shortNames[tostring(location[flag].Name)] or
                                  tostring(location[flag].Name)
            end
        ...
        function types:Slider(name, options, callback)
            ...
            check:FindFirstChild(name).Container.MouseEnter:connect(
                function()
                        ...
                        renderSteppedConnection =
                            game:GetService('RunService').RenderStepped:connect(
                                function()
                                    ...
                                    local percent =
                                        (mouse.X -
                                            overlay.Container.AbsolutePosition.X) /
                                            (overlay.Container.AbsoluteSize.X)
                                    percent = math.clamp(percent, 0, 1)
                                    percent =
                                        tonumber(string.format("%.2f", percent))

                                    overlay.Container.Button.Position =
                                        UDim2.new(math.clamp(percent, 0, 0.99),
                                                  0, 0, 1)

                                    local num = min + (max - min) * percent
                                    local value =
                                        (precise and num or math.floor(num))
                    ...
                    mouseDownConnection =
                        check:FindFirstChild(name).Container.Button
                            .MouseButton1Down:connect(update)
                    mouseUpConnection =
                        game:GetService("UserInputService").InputEnded:connect(
                            function(a, b)
                                if a.UserInputType ==
                                    Enum.UserInputType.MouseButton1 and
                                    (mouseDownConnection.Connected) then
                                    disconnect()
                                end
                            end)
                end)
            ...
            if default ~= min then
                ...
                overlay.Container.Button.Position =
                    UDim2.new(math.clamp(percent, 0, 0.99), 0, 0, 1)
        ...
        function types:SearchBox(text, options, callback)
            ...
            local function rebuild(text)
                ...
                for i, child in next,
                                box:FindFirstChild('Box').Container:GetChildren() do
                ...
                if #text > 0 then
                    for i, v in next, list do
                        if string.sub(string.lower(v), 1, string.len(text)) ==
                            ...
                            local button =
                                library:Create('TextButton', {
                                    ...
                                    TextColor3 = library.options.textcolor,
                                    BorderColor3 = library.options.bordercolor,
                                    TextStrokeTransparency = library.options
                                        .textstroke,
                                    TextStrokeColor3 = library.options
                                        .strokecolor,
                            ...
                            button.MouseButton1Click:connect(
                                function()
                                    ...
                                    box:FindFirstChild('Box').Container
                                        .ScrollBarThickness = 0
                                    for i, child in next, box:FindFirstChild(
                                                        'Box').Container:GetChildren() do
                                        if (not child:IsA('UIListLayout')) then
                                            child:Destroy();
                                        end
                                    end
                                    box:FindFirstChild('Box').Container:TweenSize(
                                        UDim2.new(1, 0, 0, 0), 'Out', 'Quad',
                                        0.25, true)
                                end)
                        end
                    end
                ...
                box:FindFirstChild('Box').Container:TweenSize(
                    UDim2.new(1, 0, 0, y), 'Out', 'Quad', 0.25, true)
                box:FindFirstChild('Box').Container.CanvasSize =
                    UDim2.new(1, 0, 0, (20 * (#c)) - 20)
            end
        ...
        function types:Dropdown(name, options, callback)
            ...
            button.MouseButton1Click:connect(
                function()
                    ...
                    check:FindFirstChild('dropdown_lbl'):WaitForChild(
                        'Selection').TextColor3 = Color3.fromRGB(60, 60, 60);
                    check:FindFirstChild('dropdown_lbl'):WaitForChild(
                        'Selection').Text = name;
                    local c = 0;
                    ...
                    for i, v in next, list do
                        ...
                        btn.MouseButton1Click:connect(
                            function()
                                check:FindFirstChild('dropdown_lbl')
                                    :WaitForChild('Selection').TextColor3 =
                                    library.options.textcolor
                                check:FindFirstChild('dropdown_lbl')
                                    :WaitForChild('Selection').Text = btn.Text;
                    ...
                    local function isInGui(frame)
                        local mloc = game:GetService('UserInputService')
                                         :GetMouseLocation();
                        local mouse = Vector2.new(mloc.X, mloc.Y - 36);

                        local x1, x2 = frame.AbsolutePosition.X,
                                       frame.AbsolutePosition.X +
                                           frame.AbsoluteSize.X;
                        local y1, y2 = frame.AbsolutePosition.Y,
                                       frame.AbsolutePosition.Y +
                                           frame.AbsoluteSize.Y;

                        return (mouse.X >= x1 and mouse.X <= x2) and
                    ...
                    input =
                        game:GetService('UserInputService').InputBegan:connect(
                            function(a)
                                if a.UserInputType ==
                                    Enum.UserInputType.MouseButton1 and
                                    (not isInGui(container)) then
                                    check:FindFirstChild('dropdown_lbl')
                                        :WaitForChild('Selection').TextColor3 =
                                        library.options.textcolor
                                    check:FindFirstChild('dropdown_lbl')
                                        :WaitForChild('Selection').Text =
                                        location[flag];

                                    container:TweenSize(UDim2.new(1, 0, 0, 0),
                                                        'In', 'Quad', 0.15, true)
                                    wait(0.15)

                                    game:GetService('Debris'):AddItem(container,
                                                                      0)
                                    input:disconnect();
                                end
                            end)
            ...
            local function reload(self, array)
                ...
                check:WaitForChild('dropdown_lbl').Selection.Text =
                    location[flag]
    ...
    local function isreallypressed(bind, inp)
        ...
        if typeof(key) == "Instance" then
            if key.UserInputType == Enum.UserInputType.Keyboard and inp.KeyCode ==
                key.KeyCode then
                return true;
            elseif tostring(key.UserInputType):find('MouseButton') and
                inp.UserInputType == key.UserInputType then
                return true
            end
        end
...
local w = library:CreateWindow("Car Crushers 2")
w:Section("Features")

local t = w:Toggle("Autofarm", {flag = "ccFarm"})
local t = w:Toggle("Auto Escape", {flag = "heliFarm"})
...
spawn(function()
    ...
    if not game:GetService("MarketplaceService"):UserOwnsGamePassAsync(
        game.Players.LocalPlayer.UserId, 2465866) then
        for i, v in pairs(info:GetChildren()) do
            if v:findFirstChild("VipOnly") then
                if v:findFirstChild("Price") then
                    if v.VipOnly.Value then v.Price.Value = 0 end
                end
            end
        end
    end
    ...
    while true do
        wait()
        if w.flags.ccFarm == true then
            game.Players.LocalPlayer.Character:MoveTo(
                Vector3.new(144.74382, 33.0044632, -539.426025))
            local info = game.ReplicatedStorage.VehicleInformation
            ...
            for i, car in pairs(info:GetChildren()) do
                ...
                if price > bestPrice and price <= money and
                    car.TokenRequirement.Value <= plr.leaderstats.Tokens.Value then
                    bestName = name
                    bestPrice = price
                end
            end

            game.ReplicatedStorage.rE.SaveCustoms:FireServer(
                {true, "Institutional white", "Platinum"}, bestName)
            wait(0.1)
            game.ReplicatedStorage.rF.SpawnVehicle:InvokeServer(bestName)

            for i = 1, 10 do
                if workspace.CarCollection:findFirstChild(
                    game.Players.LocalPlayer.Name) then
                    if workspace.CarCollection[game.Players.LocalPlayer.Name]
                        .Car.PrimaryPart ~= nil then
                        workspace.CarCollection[game.Players.LocalPlayer.Name]
                            .Car:SetPrimaryPartCFrame(
                            CFrame.new(141.74382, -325.0044632, -539.426025))
                        wait()

                        function checkpart(obj)
                            if obj.ClassName == "Part" or obj.ClassName ==
                                "MeshPart" or obj.ClassName == "Wedge" then
                                return true
                            else
                                return false
                            end
                        ...
                        if workspace.CarCollection:findFirstChild(
                            game.Players.LocalPlayer.Name) then
                            local carparts =
                                workspace.CarCollection[game.Players.LocalPlayer
                                    .Name].Car:getDescendants()
                            for i = 1, #carparts do

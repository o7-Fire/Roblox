...
while wait(0.3) do
    ...
    for i = 1,10 do
        if not enemy.Character:FindFirstChild("HumanoidRootPart") then break end
        if enemy == nil then break end
        teleportBehindTarget(enemy)
        wait()
        currentcamera.CFrame = CFrame.new(currentcamera.CFrame.p, enemy.Character.HumanoidRootPart.Position)

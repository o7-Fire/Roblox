    ...
    if LP:DistanceFromCharacter(Vector3.new(0,0,0)) > MAX_DISTANCE then
        ...
        for _,player in pairs(game.Players:GetChildren()) do
            if player.Character ~= nil then
                ...
                if enemyHRP ~= nil and LP:DistanceFromCharacter(enemyHRP.Position) <= closestPos and player ~= LP then
                    closestPos = LP:DistanceFromCharacter(enemyHRP.Position)
                    closest = player
                end
            end
        end
        if closest ~= nil and closest.Character:FindFirstChild("HumanoidRootPart") then
            local closestHRP = closest.Character:FindFirstChild("HumanoidRootPart")
            HUMANOID:MoveTo(Vector3.new(
                (HRP.CFrame.X - closestHRP.CFrame.X) + HRP.CFrame.X,
                HRP.CFrame.Y,
                (HRP.CFrame.Z - closestHRP.CFrame.Z) + HRP.CFrame.Z)
            )
        end
    end
